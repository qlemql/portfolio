...
## 미착수 — OG 이미지가 모노크롬 정리에서 빠졌다 (2026-07-30 발견)

**우선순위: 낮음.** 육안 확인 3건(인쇄 쪽수 · 다크 모드 · `/projects` 카드 배경) 이후에 처리한다.

사이트 본문은 모노크롬(zinc + `--accent`)으로 정리했지만 **OG 이미지만 컬러로 남았다.**
슬랙·트위터에서는 링크 카드가 사이트보다 먼저 보이는 경우가 많아, 첫인상이 실제 사이트와
다르게 잡힌다.

### 실제 범위 — 지적된 3색보다 넓다

문제는 accent 3색뿐이 아니다. **뉴트럴 계열 자체가 slate이고, 배경 그라디언트가 두 갈래다.**
...
색이 갈린 진짜 이유가 이것이다. 상세 라우트만 자체 구현이라 공용 헬퍼를 고쳐도 따라오지
않는다. **색만 바꾸면 다음에 또 갈린다** — 색상 토큰을 `ogImage.tsx`에 상수로 빼고 상세
라우트가 그걸 import하도록 묶는 편이 재발 방지에 맞다.

### 처리 시 결정할 것

두 배경 그라디언트를 동일한 뉴트럴로 통일하면 **홈/목록 OG와 상세 OG의 시각적 구분이
사라진다.** 현재는 하늘색/앰버로 페이지 종류가 구분됐다. 통일하는 쪽이 브랜드 일관성에는
맞지만, 구분이 필요하다고 판단하면 명도 차(`#fafafa→#e4e4e7` vs `#f4f4f5→#d4d4d8`)로
가져가는 방법이 있다. 색을 다시 들이지 않고도 구분은 만들 수 있다.
