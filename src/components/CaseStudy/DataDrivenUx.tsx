    ...
    <div className="space-y-8 text-zinc-700 dark:text-zinc-300">
      <section className="rounded-lg border-l-2 border-accent bg-zinc-50 p-5 dark:bg-zinc-900/50">
        ...
        <p className="text-sm leading-7">
          {isKo
            ? "B2C 확장에 이어 소셜 로그인으로 가입 유입이 열린 직후 두 달 동안, 기획자(PO)와 디자이너(PD), 개발이 매일 아침 전날 데이터를 같이 보고 그날의 실험을 정하는 사이클을 돌렸습니다. 그 결과 견적 확인율이 58%에서 90%로, 견적→결제 전환이 0%에서 50%로, 비회원 견적 요청이 53%에서 62%로 올랐습니다. CTA 위치나 폼 필드 순서, 로딩 상태 같은 작은 결정들이 쌓인 결과였습니다."
            : "For two months right after the B2C expansion and the social-login launch opened up signups, the PO, PD, and engineering met every morning to look at the prior day's data together and decide that day's experiment. Quote views went from 58% to 90%, quote→payment conversion from 0% to 50%, and guest quote requests from 53% to 62%. It added up from small calls about CTA placement, form-field order, and loading states."}
        </p>
      </section>
      ...
      <section className="space-y-3">
        ...
        <p className="text-sm leading-7">
          {isKo
            ? "B2B 단체 여행 견적 플랫폼에서 B2C로 확장한 Ria에서, 소셜 로그인으로 가입 진입 허들을 한 번 무너뜨린 직후였습니다. 가입한 사용자가 그다음 단계인 견적과 결제에서 어디서 멈추는지가 다음 병목이었습니다 — 당시 견적 확인율은 58%였고, 결제 기능은 있었지만 견적이 결제로 이어지지 않아 견적→결제 전환은 0%였습니다. 정답을 모를 때 가장 빠른 방법은 매일 데이터를 보고 작게 바꾸는 것이라고 판단했습니다. 팀은 기획자(PO)와 디자이너(PD), 그리고 프론트엔드 개발자인 저였습니다."
            : "This was at Ria — a B2B group-travel quote platform freshly extended to B2C — right after social login had broken the signup-entry barrier. The next bottleneck was where signed-up users actually stop, in the quote and payment steps: quote views sat at 58%, and although payments were live, no quote was converting — quote-to-payment sat at 0%. With the answer unknown, the fastest path seemed to be reading the data daily and shipping small changes. The team was the PO, the PD, and me on the frontend."}
        </p>
      </section>
      ...
      <section className="space-y-4">
        ...
        <div className="break-inside-avoid rounded-lg border-l-[3px] border-accent bg-zinc-50 p-4 dark:bg-zinc-900/60">
          ...
          <ul className="ml-5 list-disc space-y-1 text-sm leading-7">
            ...
            <li>
              {isKo
                ? "전날 데이터를 정리한 슬랙 스레드를 매일 같은 시간에 올려, PO·PD와의 의사결정 기록이 쌓이게 했습니다."
                : "I posted a summary of the prior day's data to Slack at the same time each morning, building a running decision log with the PO and PD."}
            </li>
          </ul>
        </div>
        ...
        <div className="break-inside-avoid rounded-lg border-l border-zinc-300 bg-zinc-50/50 p-4 dark:border-zinc-700 dark:bg-zinc-900/30">
          ...
          <ul className="ml-5 list-disc space-y-1 text-sm leading-7">
            <li>
              <strong>GA4:</strong>{" "}
              {isKo
                ? "가입→견적→결제 흐름(퍼널)에서 어느 단계에서 얼마나 이탈하는지(양적으로)."
                : "the signup→quote→payment funnel, where users drop and how much (quantitative)."}
            </li>
            <li>
            ...
            <li>
              {isKo
                ? "질문별로 어느 도구를 볼지(이탈 규모=GA4, 막힌 지점=Clarity, 이벤트 순서=RUM)를 첫 주에 정해 두니, 아침 15분이 곧장 가설 논의로 넘어갔습니다."
                : "We fixed in the first week which tool answers which question (drop-off size = GA4, where users get stuck = Clarity, event order = RUM), so the morning 15 minutes went straight to hypotheses."}
            </li>
          </ul>
        </div>
        ...
        <div className="break-inside-avoid rounded-lg border-l border-zinc-300 bg-zinc-50/50 p-4 dark:border-zinc-700 dark:bg-zinc-900/30">
          ...
          <ul className="ml-5 list-disc space-y-1 text-sm leading-7">
            <li>
              {isKo
                ? "결제 화면에서 CTA를 첫 화면 안으로 옮겼더니 결제 진입률이 처음으로 올랐습니다."
            ...
            <li>
              {isKo
                ? "로딩 중임을 보여주는 스켈레톤 화면과 진행 표시를 넣자, 화면 녹화(세션 리플레이)에서 보이던 '멈춘 줄 알고' 반복 클릭하다 나가는 행동이 사라졌습니다."
                : "Adding skeleton screens and progress indicators removed the behavior we kept seeing in session replays — users clicking repeatedly, assuming the screen had frozen, then leaving."}
            </li>
          </ul>
        </div>
      </section>
