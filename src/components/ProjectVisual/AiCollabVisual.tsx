  ...
  return (
    <div className="flex h-full w-full items-center gap-5">
      <div className="flex w-32 shrink-0 flex-col items-center gap-1">
        ...
        <div className="text-xs leading-none text-accent" aria-hidden="true">
          ↓
        </div>
      ...
      <div className="flex flex-1 flex-col gap-2">
        ...
        <div className="grid grid-cols-2 gap-1.5">
          {MCP_LIST.map((mcp) => (
            <span
              key={mcp}
              className="rounded-md border border-accent/30 bg-white/60 px-2 py-0.5 text-center text-xs font-medium text-zinc-700 dark:bg-zinc-900/60 dark:text-zinc-300"
            >
              {mcp}
            </span>
          ))}
